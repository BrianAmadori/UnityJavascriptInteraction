...
public class Receiver : MonoBehaviour
{
    ...
    delegate void delegate_Message(SendSceneMessage message);
    ...
    public static Dictionary<int, SendSceneMessage> messages = new Dictionary<int, SendSceneMessage>();
    ...
    [MonoPInvokeCallback(typeof(delegate_Message))]
    private static void TargetMessage(SendSceneMessage message)
    {
        if (!messages.ContainsKey(message.sceneId))
            messages.Add(message.sceneId, message);
    }
    ...
    private static void ShowMessages()
    {
        ...
        {
            Debug.Log($"id: {kvp.Key} -- content: {JsonUtility.ToJson(kvp.Value)}");
        }
